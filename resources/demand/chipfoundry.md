...
## Corrections this file implies for entries it does not own

Nothing in this list was edited by the author of this file, because the entries belong to other
parts of `resources/`. Each item names the file, what it now says, and what the evidence here says.

| Where | What it says now | What `CF-*` establishes |
|---|---|---|
| [`pricing-and-cost-to-serve.md`](pricing-and-cost-to-serve.md), `SMB-9`, last caveat | "ChipFoundry's other prices are behind 'Request a Quote'; **only the chipIgnite headline is public**." | **Wrong, and by a long way.** `chipfoundry.io` publishes a 36-block IP catalogue at $6,200–$33,900 a licence with group tiers at $8,600–$42,900, commercial SRAM at **$2,500 per project**, chipIgnite ML at **$22,250 / $45,000**, extended support at **$1,000 per five hours** and training at **$450 a seat** (`CF-16`). Only *production* pricing and *pool discounts* are by quote. |
| [`pricing-and-cost-to-serve.md`](pricing-and-cost-to-serve.md), `SMB-9`, "Bears on" | "the programme was restarted by its founders under a new company at a higher price" | Nearly right, but it elides the sequence. The new company launched its **own** product at $14,950 five months **before** it bought the chipIgnite name (`CF-2`, `CF-3`), so the price was never "raised" on an inherited product. `SMB-9` should also gain the ML comparison, which makes the +53% a three-point, ~+50% pattern rather than one number (`CF-16`). |
| [`access-terms.md`](access-terms.md), `ACC-3`, title and "Bears on" | "a reservation made by **submitting a request form, not a checkout**" … "**you cannot buy it with a card**. The purchase route is 'submitting a request to us through this form'." | **Out of date.** `chipfoundry.io/reservations` now sells a **$500 non-refundable reservation per shuttle** with "Reserve Now!" buttons linking to `platform.chipfoundry.io/reserve/ci2609` and `/reserve/ci2612`, and the published payment methods are "Credit Card (Visa, MasterCard, American Express), Wire Transfer, ACH Transfer, Purchase Order" (`CF-13`). The FAQ the entry quotes is the stale page. The open-access audit's self-service score for ChipFoundry should be revisited. |
| [`access-terms.md`](access-terms.md), `ACC-3`, last caveat | "A page titled `chipfoundry.io/terms` returns HTTP 404 … **the terms themselves were not read**." | The terms exist under three other paths and have now been read in full: `/terms-of-service`, `/commercial-terms` ("Last Updated: November 13, 2025") and `/payment-terms` ("Last Updated: April 22, 2025"). They are reachable from `chipfoundry.io/sitemap.xml` (`CF-13`). |
| [`access-terms.md`](access-terms.md), `ACC-3`, first caveat | "The FAQ text is internally inconsistent about which company is which … the wording is not [clear]." | `CF-2` and `CF-3` explain why: the FAQ was written for **ChipCreate** and only partly rewritten at the September 2025 rebrand. It still names the CC-era shuttles ("CC2509", "CC2511") and the CC-era schedule. The inconsistency is an artefact of the rename, not of sloppy drafting about Efabless. |
| [`open-programme-growth.md`](open-programme-growth.md), `OPG-9`, caveats | The 2026-06-01 Wayback capture of `ci2605/metrics` is marked **Partial**, "reported to us by a delegated agent … not re-fetched"; and the $14,950 price and the refund clause are marked **Partial**. | **All three are now Verified.** The capture was re-fetched and reads exactly `{"shuttle_name":"CI2605","interest":95,"planned":45,"reserved":34,"committed":29}`; the price and the refund clause were read directly off the live FAQ (`CF-6`, `CF-8`). `PAY-7` already upgraded the latter two; `OPG-9`'s own caveat was not updated to match. |
| [`open-programme-growth.md`](open-programme-growth.md), `OPG-9`, heading | "ChipFoundry, **the successor**, publishes live per-shuttle demand" | "Successor" is the framing this file corrects (`CF-1`, `CF-2`, `CF-3`). `OPG-9` also now has a **dated series** rather than a single reading, and a second archived capture for `ci2609` (2026-09-01: `69 / 34 / 18 / 10`). |
| [`payment-growth.md`](payment-growth.md), opening paragraph | "Efabless chipIgnite **and its successor** ChipFoundry.io" | Same correction. The file's revenue table header, "chipIgnite → ChipFoundry", reads as one continuous programme; it is two companies with a five-month overlap in which ChipFoundry taped out its own first shuttle under its own name. |
| [`payment-growth.md`](payment-growth.md), `PAY-8`, table | "CI 2511 | 2025-11" as the tapeout date | The actual tapeout was **2025-12-15**, per ChipFoundry's own `status` API with `is_projected: false`. The 2025-11 date is the *originally advertised* one, which slipped twice (`CF-7`). The year of tapeout is unaffected. |
| [`payment-growth.md`](payment-growth.md), `PAY-8`, caveats | "**at least five** ChipFoundry-era slots are Tiny Tapeout's own purchases" | It is **exactly five**, and they are identifiable: CI-2509, CI-2511, CI-2605 (×2) and CI-2609, from Tiny Tapeout's own chip table. More importantly, **ChipFoundry states that it subsidises "the cost of fabrication for a portion of Tiny Tapeout projects"** (`CF-9`), so the double-count correction is not the only problem with those five slots — part of that money was never paid. |
