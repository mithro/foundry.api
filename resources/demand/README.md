...
| [`pcb-industry-comparables.md`](pcb-industry-comparables.md) | `PCB-1` … `PCB-14` | Audited PCB makers' gross margin by order size, customer counts, concentration and capital intensity — the closest industry parallel to selling fabrication online to a long tail. `PCB-1`…`PCB-6` are the first pass (2026-09-19); `PCB-7`…`PCB-14` are the second pass (2026-09-25), which found four more companies publishing a margin split by order size and superseded the first pass's verdict |
| [`other-industry-margins.md`](other-industry-margins.md) | `OIM-1` … `OIM-7` | The same disclosure as `SMB-1`, hunted outside PCBs: capital-owning manufacturers that publish margin split by order size, batch size, customer type or channel. Printing, pharmaceutical contract manufacturing, a MEMS wafer foundry, metals processing and freight. **Contains the reverse finding — two CDMOs where the large batches carry the margin.** Runs its own number sequence |
| [`semiconductor-margins.md`](semiconductor-margins.md) | `SEM-1` … `SEM-8` | Where a specialty fab actually sits. `SEM-1` reads Silex's **balance sheet** out of its IPO prospectus, so the 22.7% in `LNI-17` can be turned into a return on assets; `SEM-2` records the utilisation figure a previous pass said was absent (about 60 percent, stated three times) and verifies the negative on wafer starts and ASP; `SEM-3` builds the whole distribution of margin, turnover and capital intensity across SIC 3674 and four neighbouring codes from the SEC's free XBRL data; `SEM-4` is the named foundry peer group from its own filings; `SEM-5` is X-FAB, which has five times Silex's customers and a third of its margin; `SEM-6` reconciles Silex's Swedish accounts against its Chinese parent's; `SEM-7` finds that capital intensity does **not** explain semiconductor margins; `SEM-8` is the verdict. Runs its own number sequence |
| [`software-margins.md`](software-margins.md) | `SWM-1` … `SWM-9` | The comparison `LNI-17` invites: what a software business actually earns, built as a **distribution** from the SEC's XBRL frames API rather than as a basket of famous names. Operating margin, asset turnover, return on assets, gross margin, R&D, SG&A, stock-based compensation, capex and PP&E intensity, as median and quartiles by SIC code; seventeen named comparators from their own filings; eight GAAP-versus-non-GAAP reconciliations; the DuPont decomposition that answers what operating margin a fab would need to match software's return on assets; and the finding that AWS, Microsoft and Oracle are now more capital-intensive per dollar of revenue than TSMC. Runs its own number sequence |
...
| [`programme-funding-deep.md`](programme-funding-deep.md) | `FUNDX-1` … `FUNDX-8` | The follow-up: what the money buys, and how much of it is people. CMC's five years of audited financial statements (which correct `FUND-7` and `FUND-9`), the CAD $120m FABrIC contribution, the FP7 publishable summaries filed with the Commission, the missing EUROPRACTICE IC3 grant, a staff census from the activity reports, CMC's 2018 near-closure, the Chips JU's €1.23bn portfolio, and the UK national money CORDIS cannot see. Ends with a head-count and payroll table, a funding table, cost per design recomputed, and a blocked-sources list. Runs its own number sequence |
...
| [`chipfoundry.md`](chipfoundry.md) | `CF-1` … `CF-17` | **ChipFoundry / UmbraLogic Technologies LLC** — the only open-shuttle operator that is both unsubsidised and (on the owner's account) investor-free, so the closest precedent this project has. Corrects the repository's "Efabless renamed" reading, and collects every public number: the per-shuttle commitment curve, the calendar and its slippage, the whole price list, the 20-participant minimum, and the contest and production products. Runs its own number sequence |
